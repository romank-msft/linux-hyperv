...
void hv_synic_free(void)
{
	...
	for_each_present_cpu(cpu) {
		...
		if (free_post_msg_page)
			hv_free_page(&hv_cpu->post_msg_page, HV_PAGE_ENC_ENCRYPT, "post msg page");
		if (free_synic_pages) {
			hv_free_page(&hv_cpu->hv_synic_event_page, HV_PAGE_ENC_ENCRYPT, "SynIC event page");
			hv_free_page(&hv_cpu->hv_synic_message_page, HV_PAGE_ENC_ENCRYPT, "SynIC msg page");
		}
		if (free_pv_synic_pages) {
			hv_free_page(&hv_cpu->pv_synic_event_page, HV_PAGE_ENC_DEAFULT, "pv SynIC event page");
			hv_free_page(&hv_cpu->pv_synic_message_page, HV_PAGE_ENC_DEAFULT, "pv SynIC msg page");
		}
	}
